...
import com.example.statussaver.databinding.FragmentFacebookBinding
import com.example.statussaver.databinding.FragmentWhatsappBinding
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
import java.lang.Exception

class FacebookFragment : Fragment(),StatusAdapter.ButtonClicked {
    ...
    val launcher = registerForActivityResult(ActivityResultContracts.OpenDocumentTree()) {
        if (it != null) {
            ...
            val sp= this.requireActivity().getSharedPreferences("DATA_PATH1", Context.MODE_PRIVATE)
            val editor=sp.edit()
            editor.putString("PATH1",it.toString())
            editor.apply()
    ...
    override fun onCreateView(
        ...
        binding= DataBindingUtil.inflate(layoutInflater,R.layout.fragment_facebook,container,false)
...
//        }
    ...
    private fun readDataFromPrefs(): Boolean {
        val sp=this.requireActivity().getSharedPreferences("DATA_PATH1", Context.MODE_PRIVATE)
        val uripath=sp.getString("PATH1","")
        if(uripath!!.isEmpty()){
            return false
        }
        return true
    ...
    private fun takePermission() {
        val targetDirectory =
            "Android%2Fmedia%2Fcom.facebook.katana"
    ...
    private fun accessStorage() {
        ...
        if (result) {
            val sp=this.requireActivity().getSharedPreferences("DATA_PATH1", Context.MODE_PRIVATE)
            val uri= Uri.parse(sp.getString("PATH1",""))
            this.requireActivity().contentResolver.takePersistableUriPermission(
                uri,
                Intent.FLAG_GRANT_READ_URI_PERMISSION
            )
